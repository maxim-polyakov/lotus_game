...
public class ShopController {
    ...
    public ResponseEntity<SpecificCardPurchaseDto> buySpecificCard(
            @AuthenticationPrincipal GameUserDetails user,
            @Valid @RequestBody SpecificCardPurchaseRequestDto request) {
        if (user == null) return ResponseEntity.status(401).build();
        return ResponseEntity.ok(shopService.buySpecificCard(user.getId(), request));
    }
}
