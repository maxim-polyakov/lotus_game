    ...
    <div className="modal-overlay" onClick={() => setNotification(null)}>
      <div className="modal-content friend-online-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2>{notification.title || '╨Ф╤А╤Г╨│ ╨▓ ╤Б╨╡╤В╨╕'}</h2>
          <button type="button" className="btn btn-secondary btn-sm" onClick={() => setNotification(null)}>├Ч</button>
        </div>
        <div className="modal-body">
          <p>{notification.message || '╨Т╨░╤И ╨┤╤А╤Г╨│ ╨▓╨╛╤И╤С╨╗ ╨▓ ╨╕╨│╤А╤Г'}</p>
          <div className="hero-drop-actions">
            <button type="button" className="btn btn-primary" onClick={() => setNotification(null)}>╨Ю╨║</button>
          </div>
        </div>
      </div>
    </div>
